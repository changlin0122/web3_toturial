...
!devlopmentChains.includes(network.name)
  ...
  : describe("test fundme contract", async function () {
      ...
      it("window closed, value greater than minimum value, fund failed", async function () {
        ...
        ).to.be.revertedWith("windoe is closed"); // wei
      });
      ...
      it("window open, value is gteater than minimum, fund success", async function () {
      ...
      it("not onwer, windowClose, target reached, getFund failed", async function () {
      ...
      it("window opened, target reached", async function () {
        await fundMe.fund({ value: ethers.parseEther("1") });
      ...
      it("window open, target reached, funder does not hav balance", async function () {
        await fundMe.fund({ value: ethers.parseEther("0.1") });
        await helpers.time.increase(200);
        await helpers.mine();
